...
- Starting the server on a port that is already in use now fails immediately with a clear error and a `--port` hint, instead of printing the full success banner (URLs, database path) before uvicorn's bind error.
